...
// РЕАЛИЗАЦИЯ функции поиска нижней точки
...
// ПАРАЛЛЕЛЬНЫЙ поиск с default(none)
...
void PetersonGrahamScannerOMP::ParallelMergeSort(PointSet &points, int left, int right, const Point2D &origin) {
  ...
  // Исправляем предупреждение о приоритете операций
  int mid = left + ((right - left) / 2);

// ПАРАЛЛЕЛЬНЫЕ задачи с default(none)
...
// ПАРАЛЛЕЛЬНАЯ сортировка с default(none)
...
bool PetersonGrahamScannerOMP::RunImpl() {
  ...
  // Алгоритм Грэхема (ПОСЛЕДОВАТЕЛЬНЫЙ)
